    ...
    methods(Static)
        ...
        function exportMetricData()
            ...
            if(ischar(userSelectedFiles))
               ...
               exportMetrics.exportTable(tableOfMetrics, userSelectedFilePath,userSelectedFiles);
            else
        ...
        function exportTable(tableOfMetrics, userSelectedFilePath,userSelectedFiles)
            ...
            labels = tableOfMetrics{1, "0_L5_SACC"}{1}.Properties.VariableNames;

            % Set Initial Export Table Headers
            columnTitles = {'SubjectID', 'StudyName', 'Movement', 'MovementIndex'};
           ...
           for section = 1:size(tableOfMetrics, 2)
                for movementIndex = 1: sectionMovementCounts(section)
                    rowNum = rowNum + 1;
                    newTable.Movement(rowNum) = char(tableOfMetrics.Properties.VariableNames(section));
                    newTable.MovementIndex(rowNum) = movementIndex;
                    newTable.SubjectID(rowNum) = char(splitFilename(1));
                    newTable.StudyName(rowNum) = strcat(char(splitFilename(3)), char(splitFilename(4)));
                    for row = 1:3
           ...
           exportPath = strsplit(userSelectedFilePath, "\");
           exportPath = exportPath(1:end-2);
           exportPath = strrep(strjoin(exportPath, '\\'), "ANALYSIS_METRICS", "EXPORTED_METRICS");
           
           mkdir(exportPath)
           writetable(newTable, fullfile(exportPath, strrep(userSelectedFiles, ".mat", ".xlsx")));
        end
    end
end
